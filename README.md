...
Schema v17 adds conversation memory policies, sessions, session participants, turns and turn events, summaries and summary versions, memory consents, memory items and their versions/events/embeddings, memory retrieval profiles/runs/results, chat context assemblies/items, chat orchestration runs/grounded responses/citations/issues, and memory evaluation suites/fixtures/runs/metrics plus a manifest table — 26 tables, all referencing existing inference-runtime and RAG tables where relevant — no second inference runtime, no second model loader, no second vector-index implementation, no duplicate public chat endpoint. Phase 17 asks whether a bounded, consent-aware memory of a conversation can be built, inspected, corrected, and deleted without ever becoming hidden permanent profiling — never whether memory improves answer quality. The default session mode (`private_no_persist`) retains nothing after a session ends, verified directly: after a full message round-trip, stored turn content, summaries, and memory items are all empty. Long-term memory requires an explicit, active consent for user-stated facts and an explicit admin confirmation for anything the assistant merely inferred; deleted, expired, revoked, rejected, and archived memory is excluded at the retrieval query itself, never filtered after the fact, so deletion invalidates retrieval on the very next call. Memory retrieval is participant-scoped by a plain string key rather than a foreign key specifically to make cross-participant leakage structurally impossible, verified directly against an intruder participant retrieving zero results for another participant's memory. Retrieval combines keyword overlap and vector similarity, reusing Phase 16's embedding and vector-scoring code unchanged. Grounded chat orchestration combines conversation state, memory evidence, and RAG evidence into one bounded, budget-enforced context — the current user message is never silently dropped to make room for anything else, and a budget that cannot be met fails closed (`blocked_context`) rather than fabricating an answer. Injection-style content is blocked at the earliest possible point (memory proposal and turn validation both reuse the same safety scan) and, for anything that reaches context assembly regardless, flagged and excluded rather than followed. The public chatbot remains the unchanged placeholder throughout, verified directly after every session, memory, and orchestration operation exercised in this phase. See [docs/phase_17_report.md](docs/phase_17_report.md) and [docs/architecture.md](docs/architecture.md).
