"""MB-17: API tests for /admin/mini-brain/vision-rag.

Auth/CSRF requirements and route wiring over real HTTP. The full
retrieval workflow is already proven at the service layer in
test_mini_brain_vision_rag_service.py -- this file confirms the routes
correctly translate HTTP requests into those same service calls.
"""
...
VR = "/api/admin/mini-brain/vision-rag"
...
async def test_routes_require_admin_auth(api_app: FastAPI) -> None:
    ...
    try:
        assert (await client.get(f"{VR}/sessions")).status_code == 401
        assert (await client.get(f"{VR}/diagnostics")).status_code == 401
    finally:
        await client.aclose()


async def test_diagnostics_discloses_no_writes_no_inference(api_app: FastAPI) -> None:
    client, headers = await authenticated_client(api_app)
    try:
        response = await client.get(f"{VR}/diagnostics", headers=headers)
        assert response.status_code == 200
        body = response.json()
        assert body["dataset_studio_writes_performed"] is False
        assert body["document_workspace_writes_performed"] is False
        assert body["vision_model_inference_performed"] is False
        assert body["requires_certified_dataset"] is True
...
async def test_create_session_requires_csrf_header(api_app: FastAPI) -> None:
    ...
    try:
        response = await client.post(
            f"{VR}/sessions", json={"multimodal_dataset_session_public_id": "x", "query": "hello"},
        )
...
async def test_full_workflow_over_http(api_app: FastAPI) -> None:
    ...
    try:
        seed_admin_id = AdminRepository(api_app.state.settings.resolved_database_path).create_admin(
            AdminCreate(username="vr-seed-admin", display_name="S", password="password12345")
        ).public_id
        mds_id = await _seed_certified_multimodal_dataset(api_app, seed_admin_id)

        response = await client.post(
            f"{VR}/sessions",
            json={"multimodal_dataset_session_public_id": mds_id, "query": "Where is the river located?"},
            headers=headers,
        )
        assert response.status_code == 200, response.text
        session_id = response.json()["public_id"]
        assert response.json()["stage"] == "query_session"

        for path in (
            "text-retrieval", "ocr-retrieval", "image-retrieval", "object-retrieval",
            "knowledge-graph-retrieval", "evidence-fusion", "answer", "quality", "hallucination-check",
            "report",
        ):
            response = await client.post(f"{VR}/sessions/{session_id}/{path}", headers=headers)
        ...
        response = await client.post(
            f"{VR}/sessions/{session_id}/admin-review", json={"decision": "approve"}, headers=headers,
        )
        assert response.status_code == 200, response.text
        assert response.json()["stage"] == "closed"

        response = await client.get(f"{VR}/sessions/{session_id}/evidence", headers=headers)
        assert response.status_code == 200
        assert len(response.json()["items"]) > 0

        response = await client.get(f"{VR}/rag-memory", headers=headers)
        assert response.status_code == 200
...
async def test_evidence_fusion_before_retrieval_returns_error_status(api_app: FastAPI) -> None:
    client, headers = await authenticated_client(api_app)
    try:
        seed_admin_id = AdminRepository(api_app.state.settings.resolved_database_path).create_admin(
            AdminCreate(username="vr-seed-admin2", display_name="S", password="password12345")
        ).public_id
        mds_id = await _seed_certified_multimodal_dataset(api_app, seed_admin_id)

        response = await client.post(
            f"{VR}/sessions",
            json={"multimodal_dataset_session_public_id": mds_id, "query": "Where is the river?"},
            headers=headers,
        )
        session_id = response.json()["public_id"]

        response = await client.post(f"{VR}/sessions/{session_id}/evidence-fusion", headers=headers)
