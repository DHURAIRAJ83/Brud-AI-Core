"""Operator CLI for Phase 11 base-training experiments.
...
from backend.models.base_training import BaseTrainingRunCreate
from backend.services.base_training_service import BaseTrainingService

CLI_ADMIN_ID = "00000000-0000-0000-0000-0000000000c1"


def _service() -> BaseTrainingService:
    settings = get_settings()
    return BaseTrainingService(
        BaseTrainingRepository(settings.resolved_database_path),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Brud AI base-training experiment CLI")
    ...
    tokenizer_parser = subparsers.add_parser("tokenizer-evaluate")
    tokenizer_parser.add_argument("experiment_public_id")
    ...
    try:
        if args.command == "list":
        ...
        elif args.command == "tokenizer-evaluate":
            _print(service.evaluate_tokenizer(args.experiment_public_id, CLI_ADMIN_ID))
        elif args.command == "create-run":
            if not _confirm(f"About to create a run for {args.experiment_public_id}.", "create"):
                print("Cancelled.", file=sys.stderr)
                return 1
            payload = BaseTrainingRunCreate(run_label=args.label)
            ...
            if not _confirm(
                f"About to select/reject a base candidate for {args.experiment_public_id}.",
                "select",
            ):
                print("Cancelled.", file=sys.stderr)
                return 1
        ...
        print(f"Base training command failed: {exc}", file=sys.stderr)
        return 1
    return 0
