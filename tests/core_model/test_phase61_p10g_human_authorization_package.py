"""Phase 61 - P10-G: Human Authorization Package Acquisition & Pre-Activation Verification Test Suite."""
...
class TestPhase61P10GHumanAuthorizationPackage(unittest.TestCase):

    def setUp(self):
        self.secret_key = "BRUD_P10G_SECRET_KEY_2026"
    ...
    def test_01_absent_training_token_acquisition_fails_closed(self):
        with self.assertRaises(TrainingAuthorizationError):
            self.training_gate.verify_authorization(None)

    def test_02_absent_promotion_token_acquisition_fails_closed(self):
    ...
    def test_03_absent_public_chat_token_acquisition_fails_closed(self):
    ...
    def test_04_absent_recovery_token_acquisition_fails_closed(self):
        with self.assertRaises(RecoveryAuthorizationError):
            self.recovery_gate.verify_recovery_token(None, "rel-01", "snap-01", "m1")

    def test_05_absent_compliance_token_acquisition_fails_closed(self):
    ...
    def test_06_fake_token_signature_fails_closed(self):
        token = SignedTrainingAuthorizationToken(
            token_id="tok-fake-p10g", admin_public_id="fake-admin", dataset_manifest_hash="d1",
            ...
            created_at_epoch=time.time(), expires_at_epoch=time.time() + 3600, signature_hmac="invalid_hmac"
        )
        with self.assertRaises(TrainingAuthorizationError):
            self.training_gate.verify_authorization(token, ignore_runtime_flag=True)

    def test_07_expired_token_acquisition_fails_closed(self):
        token = SignedTrainingAuthorizationToken(
            token_id="tok-exp-p10g", admin_public_id="admin-dhurai", dataset_manifest_hash="d1",
            rights_gate_status="PASS", novelty_gate_status="PASS", provenance_gate_status="PASS",
    ...
    def test_08_rbac_admin_assistant_advisory_cannot_sign_or_issue_tokens(self):
    ...
    def test_09_rbac_system_operator_cannot_sign_promotion(self):
        with self.assertRaises(RBACPermissionError):
            self.rbac_engine.authorize_action("SYSTEM_OPERATOR", "PROMOTE_MODEL")

    def test_10_rbac_security_auditor_cannot_sign_training(self):
        with self.assertRaises(RBACPermissionError):
            self.rbac_engine.authorize_action("SECURITY_AUDITOR", "EXECUTE_TRAINING")

    def test_11_cross_tenant_token_acquisition_denied(self):
        with self.assertRaises(TenantAccessError):
            self.tenant_engine.authorize_tenant_access("tenant-brud-core", "tenant-unauthorized")

    def test_12_audit_chain_revalidation_verifies_continuity(self):
        self.audit_chain.append_event("P10G_ACQUISITION", "rel-01", "m1", "admin", "P10G", "PASS", "h1")
        self.assertTrue(self.audit_chain.verify_chain_integrity())

    def test_13_valid_signed_training_token_acquisition(self):
        sig = compute_training_token_signature("tok-val-p10g", "admin-dhurai", "d1", self.secret_key)
        token = SignedTrainingAuthorizationToken(
            token_id="tok-val-p10g", admin_public_id="admin-dhurai", dataset_manifest_hash="d1",
            rights_gate_status="PASS", novelty_gate_status="PASS", provenance_gate_status="PASS",
            holdout_gate_status="PASS", token_accounting_status="PASS", human_approval_signature="sig_h",
    ...
    def test_14_valid_signed_promotion_token_acquisition(self):
        sig = compute_promotion_token_signature("tok-prom-p10g", "admin-dhurai", "cand-m1", "d1", self.secret_key)
        token = SignedPromotionAuthorizationToken(
            token_id="tok-prom-p10g", admin_public_id="admin-dhurai", candidate_model_id="cand-01",
    ...
    def test_15_valid_signed_public_chat_token_acquisition(self):
        sig = compute_public_chat_admission_signature("tok-pc-p10g", "admin-dhurai", "rel-01", "cand-m1", self.secret_key)
        token = SignedPublicChatAdmissionToken(
            token_id="tok-pc-p10g", admin_public_id="admin-dhurai", release_id="rel-01", candidate_model_hash="cand-m1",
    ...
    def test_16_valid_signed_recovery_token_acquisition(self):
        sig = compute_recovery_token_signature("req-rec-p10g", "admin-dhurai", "rel-01", "snap-01", self.secret_key)
        token = SignedRecoveryAuthorizationToken(
            recovery_request_id="req-rec-p10g", admin_public_id="admin-dhurai", release_id="rel-01", snapshot_hash="snap-01",
    ...
    def test_17_valid_signed_compliance_token_acquisition(self):
        sig = compute_compliance_token_signature("req-cmp-p10g", "pkg-01", "hash-01", "tenant-brud-core", "admin-dhurai", self.secret_key)
        token = SignedComplianceCertificationToken(
            certification_request_id="req-cmp-p10g", package_id="pkg-01", package_hash="hash-01",
    ...
    def test_20_adversarial_wrong_dataset_hash_promotion_fails_closed(self):
        sig = compute_promotion_token_signature("tok-prom-badhash-g", "admin-dhurai", "cand-m1", "wrong-dataset-hash", self.secret_key)
        token = SignedPromotionAuthorizationToken(
            token_id="tok-prom-badhash-g", admin_public_id="admin-dhurai", candidate_model_id="cand-01",
    ...
    def test_21_adversarial_wrong_tokenizer_hash_promotion_fails_closed(self):
        sig = compute_promotion_token_signature("tok-prom-badtok-g", "admin-dhurai", "cand-m1", "d1", self.secret_key)
        token = SignedPromotionAuthorizationToken(
            token_id="tok-prom-badtok-g", admin_public_id="admin-dhurai", candidate_model_id="cand-01",
    ...
    def test_22_adversarial_wrong_model_hash_public_chat_fails_closed(self):
        sig = compute_public_chat_admission_signature("tok-pc-badmodel-g", "admin-dhurai", "rel-01", "wrong-model-hash", self.secret_key)
        token = SignedPublicChatAdmissionToken(
            token_id="tok-pc-badmodel-g", admin_public_id="admin-dhurai", release_id="rel-01", candidate_model_hash="wrong-model-hash",
            dataset_manifest_hash="d1", human_admission_signature="sig_h", created_at_epoch=time.time(),
            expires_at_epoch=time.time() + 3600, signature_hmac=sig
        )
        rel = ProductionReleaseRecord(
    ...
    def test_23_adversarial_stale_token_fails_closed(self):
        sig = compute_training_token_signature("tok-stale-g", "admin-dhurai", "d1", self.secret_key)
        token = SignedTrainingAuthorizationToken(
            token_id="tok-stale-g", admin_public_id="admin-dhurai", dataset_manifest_hash="d1",
            rights_gate_status="PASS", novelty_gate_status="PASS", provenance_gate_status="PASS",
            holdout_gate_status="PASS", token_accounting_status="PASS", human_approval_signature="sig_h",
    ...
    def test_24_adversarial_revoked_key_token_fails_closed(self):
        rec = self.secret_mgr.register_key("TEST_REVK_G", "REVOKED_SECRET_16B_G")
