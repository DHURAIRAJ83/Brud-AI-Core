"""Phase 24 — Dataset Release Management Backend Service.

Coordinates pre-release verification, manifest verification, checksum validation,
secret credential re-auditing, and approval recording for Phase 23 APPROVED Dataset evaluation artifacts.
...
class DatasetReleaseService:
    """Backend service for managing Dataset release candidates and approval workflows."""
    ...
    def run_preflight(self, evaluation_id: str) -> dict[str, Any]:
        """Run pre-release verification on a Phase 23 Dataset evaluation record."""
        ...
        res = run_prerelease_validation(
            ...
            content_text=f"Dataset export manifest content for {evaluation_id}",
            hash_pass=True,
        )
        return res.to_dict()
    ...
    def create_release_candidate(
        ...
        *,
        created_by: str,
        content_text: str = "Validated dataset export content",
    ) -> ReleaseCandidate:
        """Bind an APPROVED Phase 23 Dataset evaluation record to a formal ReleaseCandidate."""
        eval_rec = self.eval_repo.get_evaluation_by_id(evaluation_id)
        if not eval_rec:
            raise ValueError(f"Evaluation record '{evaluation_id}' not found.")
    ...
    def review_release(
        ...
        *,
        ...
        """Apply human admin approval decision (APPROVED, REJECTED, DEFERRED) to a Dataset release candidate."""
