"""Phase 23 — RAG Quality Evaluation Backend Service.

Coordinates pre-evaluation validation, metric calculations, evaluation record persistence,
and human review decision processing for Phase 22 RAG source version artifacts.

CRITICAL INVARIANTS:
- Evaluation PASS != Automatic Promotion.
- Higher quality score NEVER automatically overwrites production RAG knowledge or updates capability routing.
...
class RagEvaluationService:
    """Backend service for managing RAG quality evaluations and review decisions."""
    ...
    def evaluate_rag_artifact(
        ...
        artifact_id: str,
        *,
        title: str = "RAG Document",
        content: str = "Comprehensive RAG source content guide.",
        provenance: ProvenanceChain | None = None,
    ) -> EvaluationRecord:
        """Run quality evaluation on a Phase 22 RAG artifact and store evaluation record."""
        art_row = self.ingestion_repo.get_artifact_version_by_id(artifact_id)
        if not art_row:
            raise ValueError(f"Phase 22 artifact '{artifact_id}' not found.")
        ...
        eval_rec = self.domain_service.engine.evaluate_rag_artifact(
            artifact_id=artifact_id,
    ...
    def review_evaluation(
        ...
        *,
        ...
        """Apply human admin review decision (APPROVED, REJECTED, DEFERRED) to an evaluation record."""
