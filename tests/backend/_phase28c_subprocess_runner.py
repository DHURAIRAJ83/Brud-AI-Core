"""Phase 2.8C: genuinely separate OS processes for proving real,
fresh-process checkpoint recovery. Not a pytest module itself --
invoked as a standalone script (`subprocess.run([sys.executable,
__file__, mode, ...])`) by
tests/backend/test_phase28c_checkpoint_resume_recovery.py.

Two independent entry points, run as two independent processes by the
parent test:

- `train_checkpoint_pause`: builds a real, isolated MB-22 job, runs one
  real training step, saves one real checkpoint, pauses the job, then
  exits *normally*. Prints one JSON line with everything the second
  process needs to attempt recovery.

- `recover_and_continue`: constructs a BRAND NEW `TorchTrainingAdapter`/
  `MiniBrainTrainingEngineService` pair (this process has never seen the
  first process's Python objects at all -- true fresh-process state),
  calls the real `resume()` recovery path, and reports exactly what was
  restored, whether a real resumed step executed, and whether a new
  checkpoint could be saved afterward.
"""
...
def train_checkpoint_pause(tmp_dir: str, name_suffix: str) -> None:
    ...
    svc.run_validate_authorization_stage(job_id, authorization_reason="phase 2.8c subprocess", admin_id=admin_id)
...
def recover_and_continue(tmp_dir: str, job_id: str, expect_success: str) -> None:
    from pathlib import Path
    ...
    adapter = TorchTrainingAdapter()
    assert adapter._model is None, "a freshly-constructed adapter in a NEW process must start with no model"
    svc = MiniBrainTrainingEngineService(settings, adapters={"gpu": adapter})
    ...
    except Exception as exc:  # noqa: BLE001 -- deliberately captured for the negative-path subprocess tests
    ...
    if expect_success != "true":
        # A negative-path test expected rejection but recovery succeeded --
        # let the parent process see this as a genuine failure.
...
if __name__ == "__main__":
    ...
    elif mode == "recover_and_continue":
        recover_and_continue(sys.argv[2], sys.argv[3], sys.argv[4])
    else:
