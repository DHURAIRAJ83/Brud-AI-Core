"""Phase 55 Corpus Diversity and Information Entropy Engine V3.
...
- Relative Vocabulary Growth over Phase 54 Baseline
- Dedicated accounting for NEW UNIQUE TOKENS ONLY
"""
...
class Phase55DiversityReport:
    ...
    new_unique_tokens_over_p54: int
    new_unique_words_over_p54: int
    vocabulary_expansion_rate: float
    dominance_warning: bool


class Phase55DiversityAnalyzer:
    PHASE54_UNIQUE_TOKENS = 3918
    PHASE54_UNIQUE_WORDS = 1883
    ...
    def analyze_records(
        cls,
        records: List[Any],
        phase54_vocab: Optional[Set[str]] = None,
    ) -> Phase55DiversityReport:
        if not records:
            return Phase55DiversityReport(
                total_records=0,
                total_tokens=0,
                unique_tokens=0,
                total_characters=0,
                ...
                new_unique_tokens_over_p54=0,
                new_unique_words_over_p54=0,
        ...
        # Growth over Phase 54 baseline
        new_tokens = max(0, total_tokens - cls.PHASE54_UNIQUE_TOKENS)
        if phase54_vocab is not None:
            new_words = len(set(word_counts.keys()) - phase54_vocab)
        else:
            new_words = max(0, unique_tokens - cls.PHASE54_UNIQUE_WORDS)

        growth_rate = new_tokens / max(1, cls.PHASE54_UNIQUE_TOKENS)

        # Dominance warning: flagged if any single domain exceeds 35%
        dominance_warning = any(
            (c / total_records) > 0.35 for c in domain_counts.values()
        )
        ...
        return Phase55DiversityReport(
            ...
            new_unique_tokens_over_p54=new_tokens,
            new_unique_words_over_p54=new_words,
